...
class PostController extends Controller
{
    ...
    public function store(Request $request){
        ...
        # 3. Save dthe categories to the category_post table
        $category_post = [];
    ...
    public function edit($id){
        ...
        if(Auth::user()->id != $post->user->id){
            return redirect()->route('indes');
        }

        $all_categories = $this->category->all();

        # Get all the catery IDs of this post. Save in an array.
        $selected_categories = [];
    ...
    public function update(Request $request, $id){
        ...
        # 3. Dlete all records from the category_post related to this post
        $post->categoryPost()->delete();
        // Use the relationship Post::categoryPost() to select the reccords related to a post
        // Equivalent: DELETE FROM category_post WHERE post_id = $id

        # 4. Sace the new categories to category_post table.
        $category_post = [];
        foreach($request->category as $category_id){
            $category_post[] = ['category_id' => $category_id];
        }
